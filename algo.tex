...
See Table~\ref{tab:mbfq_init} for notation.  The MBFQ algorithm runs every $T$
...
We start by initializing each VM's allocated rate to be the minimum of its $TR$
and its guaranteed rate ($MG$).  This guarantees congestion freedom since the
