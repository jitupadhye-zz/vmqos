...
\ElsIf {($SR_{i} < 0.85 \times AR_{i}$ for the last 1second)}
\State $TR_{i} \gets 1.1 \times SR_{i}$ 
...
See Table~\ref{table:mbfq_init} for notation.  The MBFQ algorithm runs every $T$
...
\fixme{Somewhere in this section or next, we need to mention that weight of a VM
is ratio of min gurantee to link bandwidth. This point is completley missing.}
